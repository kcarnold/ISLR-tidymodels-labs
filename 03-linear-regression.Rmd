...
- `x*y` will include `x`, `y`, and their interaction
- `x:y` will include *only* the interaction between `x` and `y`.

Thus, `x*y` is short for `x:y + x + y`.

So let's expand `lm_fit2` by adding an interaction term.
